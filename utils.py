...
def slerp(v, w, alpha):
    ...
    dot = (v * w).sum(dim=-1, keepdim=True)
    theta = torch.acos(dot.clamp(-1, 1))  # Clamping to handle numerical errors
    
    # If the angle is very small, fallback to linear interpolation
    if theta.abs().max() < 1e-5:
        return (1 - alpha) * v + alpha * w
    
    # Perform Slerp
    sin_theta = torch.sin(theta)
    part1 = torch.sin((1 - alpha) * theta) / sin_theta * v
    part2 = torch.sin(alpha * theta) / sin_theta * w
    return part1  + part2
